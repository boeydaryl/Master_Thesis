...
dir1 <- "/Volumes/scRNAseq_1/DESeq runs/p1p2p3vsp4p5/"

#### To load sample metadata and search for related salmon files #####
samples <- read.csv(file.path(dir1, "p1p2p3vsp4p5.csv"), header = TRUE)
...
names(files) <- paste0(samples$Well.number, "_sample", 1:235)
...
write.csv(res, file = 
            "/Volumes/scRNAseq_1/DESeq runs/p1p2p3vsp4p5/deseq_LOvsEO.csv")
